...
app.post('/analyze-data', async (req, res): Promise<void> => {
  try {
...
function processDataInChunks(data: any[], headers: string[], maxChunkSize: number = 5000): any {
...
function generateDataStatistics(data: any[], headers: string[]): any {
...
function combineChunkStatistics(chunkStats: any[], headers: string[]): any {
...
function getRandomSamples(data: any[]): any[] {
...
function extractUniqueValues(csvData: any[], headers: string[]): Record<string, any[]> {
...
function identifyRelationships(data: any[], headers: string[], stats: any): any[] {
  const relationships = [];
...
app.post('/notify-admin', async (req, res): Promise<void> => {
  try {
...
export const api = onRequest({
  timeoutSeconds: 540, // 9 minutes - well under the 15 minute limit
  memory: '2GiB', // More memory for large dataset processing
  cors: true
}, app);